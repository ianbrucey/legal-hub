...
class ChiefEditorAgent:
    ...
    def _create_output_directory(self):
        # Check if output_path is specified in task config
        if self.task.get('output_path'):
            output_dir = self.task.get('output_path')
        else:
            # Fall back to default timestamped directory
            output_dir = "./outputs/" + \
                sanitize_filename(
