...
class PublisherAgent:
    ...
    async def publish_research_report(self, research_state: dict, publish_formats: dict, filename: str = "report"):
        """Publish research report in requested formats.

        Args:
            research_state: The research state containing data to publish
            publish_formats: Dict specifying which formats to generate
            filename: Base filename to use (without extension). Defaults to "report".
        """
        layout = self.generate_layout(research_state)
        await self.write_report_by_formats(layout, publish_formats, filename)
...
"""
    ...
    async def write_report_by_formats(self, layout:str, publish_formats: dict, filename: str = "report"):
        """Write report in requested formats with custom filename.

        Args:
            layout: The report content to write
            publish_formats: Dict specifying which formats to generate
            filename: Base filename to use (without extension). Defaults to "report".
        """
        if publish_formats.get("pdf"):
            await write_md_to_pdf(layout, self.output_dir, filename)
        if publish_formats.get("docx"):
            await write_md_to_word(layout, self.output_dir, filename)
        if publish_formats.get("markdown"):
            await write_text_to_md(layout, self.output_dir, filename)

    async def run(self, research_state: dict):
        task = research_state.get("task")
        publish_formats = task.get("publish_formats")
        # Get custom filename from task config, default to "report"
        filename = task.get("filename", "report")
        ...
        final_research_report = await self.publish_research_report(research_state, publish_formats, filename)
